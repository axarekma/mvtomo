...
class GaussianOperator(Operator):
    ...
    def conv_pytorch(self,volume):
        return gaussian_blur_sep_nn(volume,kernel_size = self.kernel_size, sigma = self.sigma)   
    ...
    #     if isinstance(self.A,ts.Operator.Operator):
    #         return self.mod_fp(volume, out)
    #     elif isinstance(self.A,ts.Operator.BackprojectionOperator):
    #         return self.mod_bp(volume, out)
